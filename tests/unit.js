  ...
  const TESTS = [
    ...
    // --- condition language --------------------------------------------------------------
    {
      ...
      run(run, assert) {
        ...
        assert(at(10).activeById.has(SHORT), 'short active at 10s (inclusive lower bound)');
        assert(at(29).activeById.has(SHORT), 'short active at 29s');
        assert(!at(30).activeById.has(SHORT), 'short inactive at 30s (exclusive upper bound)');

        assert(!at(29).activeById.has(MEDIUM), 'medium+ inactive at 29s');
        assert(at(30).activeById.has(MEDIUM), 'medium+ active at 30s');
        assert(at(85).activeById.has(MEDIUM), 'medium+ active at 85s — a value the sheet '
    ...
    // --- tiers, variants, stacking, exclusion ---------------------------------------------
    {
      ...
      run(run, assert) {
        const ID = 'gladiator-s-guile-bonus';
    ...
    {
      name: 'Role variants select exactly one payload',
      run(run, assert) {
        const ID = 'm28-voidtouched-set-bonus-single-dps-combat-medium-plus-combat';
        ...
        assert(near(healer.statOf(ID, 'overall_healing'), 0.06), 'healer -> overall healing');
        assert(near(tank.statOf(ID, 'incoming_damage'), -0.06), 'tank -> incoming damage');
        assert(dps.statOf(ID, 'overall_healing') === undefined,
          'and only the matching variant applies');
      },
    },
    {
      name: 'A two-piece set needs two pieces',
      run(run, assert) {
        const ID = 'm28-voidtouched-set-bonus';
    ...
    },
    {
      ...
      run(run, assert) {
        const ID = 'mount-vortex-panther-necrotic-bonus-artifact-call';
    ...
    },
    {
      ...
      run(run, assert) {
        ...
        const visit = (bonus) => {
          walk(bonus.when);
          (bonus.variants ?? []).forEach((v) => walk(v.when));
        };
        for (const item of window.NW_ITEMS) (item.bonuses ?? []).forEach(visit);
        for (const set of window.NW_BONUSES) set.effects.forEach(visit);
