...
import type { BonusSet, ConditionWhen, Slot } from '../../src/types';

const paramSlot = (id: string, path: string): Slot => ({
  id, label: id, section: 'options', type: 'build_parameter', paramType: 'boolean', path,
});
...
describe('catalog.validateSlots', () => {
  ...
  it('reports a custom parameter shadowing a bare compound BuildContext field', () => {
    ...
    expect(findings.some((f) => /shadows a BuildContext field/.test(f.message))).toBe(true);
  });
  ...
  it('does not flag legitimate nesting into a compound field', () => {
    const findings = catalog.validateSlots([paramSlot('a', 'toggles.myFeature')]);
    expect(findings).toEqual([]);
  });

  it('item_picker slots are ignored entirely (no path to check)', () => {
    const findings = catalog.validateSlots([
      { id: 'gear.head', label: 'Head', section: 'gear', type: 'item_picker', filter: 'gear_head' },
    ]);
    expect(findings).toEqual([]);
  });
});
...
describe('catalog.validate: param condition lint', () => {
  const setWith = (when: ConditionWhen): BonusSet[] => [{ id: 'test-set', grants: [{ when, stats: {} }] }];
  const errorsFor = (when: ConditionWhen) => catalog.validate([], setWith(when))
    .filter((f) => f.level === 'error');
  const warningsFor = (when: ConditionWhen) => catalog.validate([], setWith(when))
  ...
  it('an unresolvable key is an error -- the condition can never be active', () => {
    ...
    expect(errors.some((f) => /not a build_parameter's path/.test(f.message))).toBe(true);
  });
  ...
  it('a list slot\'s "equals" must be one of its declared options', () => {
    const errors = errorsFor({ param: { key: 'class', equals: 'not-a-class' } });
    expect(errors.some((f) => /not one of its declared options/.test(f.message))).toBe(true);
  });
  ...
  it('warns (not errors) when a param duplicates a dedicated leaf', () => {
    ...
    expect(warningsFor(when).some((f) => /dedicated "class" condition/.test(f.message))).toBe(true);
  });

  it('warns for a toggle path, pointing at the dedicated "toggle" leaf', () => {
    const warnings = warningsFor({ param: { key: 'toggles.combat', is: true } });
    expect(warnings.some((f) => /dedicated "toggle" condition/.test(f.message))).toBe(true);
  });
