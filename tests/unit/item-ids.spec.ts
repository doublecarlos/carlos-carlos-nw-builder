...
const schema: Schema = {
  ...
  statKeys: ['il'],
  multiplicativeStats: [], ratingStats: [], abilityStats: [],
  ratingConversion: [], abilityContributions: [], forteSplit: {},
...
const slots: SlotsData = {
  ...
  slots: [
    { id: 'gear.ring1', label: 'Ring 1', section: 'gear', type: 'item_picker', filter: 'gear_ring' },
    { id: 'gear.ring2', label: 'Ring 2', section: 'gear', type: 'item_picker', filter: 'gear_ring' },
  ],
};

function testBuild(choices: Record<string, string>): Build {
  return {
    id: 'b', name: 'b', updated: 0, choices, values: {},
    context: {
      class: '', role: '', combatType: '', location: '', damageType: '',
      duration: 0, magnitude: 0, m32Forte: false, forte: {}, toggles: {},
    },
...
describe('item ids: two items sharing a display name', () => {
  const itemA: Item = { id: 'ring-a', name: 'Ring', filter: 'gear_ring', il: 100, maxCopies: 1 };
  const itemB: Item = { id: 'ring-b', name: 'Ring', filter: 'gear_ring', il: 200, maxCopies: 1 };
  ...
  it('count separately for maxCopies -- one of each is fine, two of the same id is not', () => {
    const oneEach = engine.resolveBuild(testDb, testBuild({ 'gear.ring1': 'ring-a', 'gear.ring2': 'ring-b' }));
    expect(oneEach.errors).toEqual([]);

    const twoOfA = engine.resolveBuild(testDb, testBuild({ 'gear.ring1': 'ring-a', 'gear.ring2': 'ring-a' }));
...
describe('item ids: renaming leaves an id-keyed reference intact', () => {
  it('a build choosing an item by id still resolves it, to the renamed item, after an overlay edit', () => {
    const created = catalog.upsert(catalog.emptyOverlay(), 'items', 'some-item', {
      id: 'some-item', name: 'Old Name', filter: 'gear_ring', il: 500,
    });
    const build = testBuild({ 'gear.ring1': 'some-item' });

    const beforeRename = engine.resolveBuild(catalog.makeDb([created]), build);
    expect(beforeRename.rows.find((r) => r.slotId === 'gear.ring1')?.item?.name).toBe('Old Name');
    ...
    const renamed = catalog.upsert(created, 'items', 'some-item', {
      id: 'some-item', name: 'New Name', filter: 'gear_ring', il: 500,
    });
    const afterRename = engine.resolveBuild(catalog.makeDb([renamed]), build);
    expect(afterRename.rows.find((r) => r.slotId === 'gear.ring1')?.item?.name).toBe('New Name');
  });
});
