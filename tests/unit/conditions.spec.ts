...
function ctx(params: Record<string, string | number | boolean>, overrides: Partial<EvalContext> = {}): EvalContext {
  return {
    duration: 0, toggles: {}, equipped: new Map(), tags: new Map(), setPieces: new Map(),
...
describe('conditions.ts param leaf', () => {
  it('number form: a half-open atLeast/below range, same as duration', () => {
    ...
    expect(evaluate({ param: { key: 'bolster', atLeast: 0.5, below: 1 } }, c)).toBe(true);
    expect(evaluate({ param: { key: 'bolster', atLeast: 0.6 } }, c)).toBe(false);
  ...
  it('string form: equals a scalar, or "is one of" an array', () => {
    ...
    expect(evaluate({ param: { key: 'tier', equals: 'silver' } }, c)).toBe(false);
    expect(evaluate({ param: { key: 'tier', equals: ['silver', 'gold'] } }, c)).toBe(true);
  });
  ...
  it('an unresolvable key fails closed, whatever comparison form is used', () => {
    ...
    expect(evaluate({ param: { key: 'does-not-exist', atLeast: 0 } }, c)).toBe(false);
    expect(evaluate({ param: { key: 'does-not-exist', is: false } }, c)).toBe(false);
  });
...
const testSlots: SlotsData = {
  ...
  slots: [{
    id: 'options.bolster', label: 'Bolster', section: 'options',
    type: 'build_parameter', paramType: 'percent', path: 'bolster', default: 0.25,
  }],
};
const emptySchema: Schema = {
  stats: [], statByKey: {}, statKeys: [], multiplicativeStats: [], ratingStats: [],
  abilityStats: [], ratingConversion: [], abilityContributions: [], forteSplit: {}, roles: {},
};
...
describe('bonus.ts collect() populates EvalContext.params', () => {
  ...
  it('a param the build has set overrides the default', () => {
    const { ctx: builtCtx } = bonus.collect(testDb, testBuild({ bolster: 0.9 }));
