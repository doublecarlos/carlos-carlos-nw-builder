...
test("the page body does not scroll -- BuildEditor and the sidebar scroll internally instead", async ({
  page,
}) => {
  ...
  const contentOverflows = await content.evaluate(
    (el) => el.scrollHeight > el.clientHeight,
  );
  const sidebarOverflows = await sidebar.evaluate(
    (el) => el.scrollHeight > el.clientHeight,
  );
