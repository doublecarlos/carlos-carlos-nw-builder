...
test("all three groups render with correct counts", async ({ page }) => {
  ...
  await expect(notInDemo.locator("summary")).toHaveText("Not in the demo (14)");
});
...
test("'not in demo' is rolled up to sections, not individual slots", async ({
  ...
  // 14 authored groups roll up well over 100 individual missing slots -- a raw per-slot list
