...
async function fillImport(page: Page, text: string) {
  await page.getByPlaceholder('Paste a build, or an array of builds…').fill(text);
  await page.getByTestId('build-header').getByRole('button', { name: 'Import', exact: true }).click();
}
...
test('a mismatched schema version is refused with a clear message', async ({ page }) => {
  await openBuilder(page);
  await openIoPanel(page);
  const envelope = await exportedEnvelope(page);
  envelope.v += 1000;
...
test('a collection bundle imported as a single build is refused as the wrong kind', async ({ page }) => {
  await openBuilder(page);
  await openIoPanel(page);
  const envelope = await exportedEnvelope(page);
