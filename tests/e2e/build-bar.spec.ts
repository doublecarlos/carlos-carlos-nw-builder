...
test("Save is disabled until dirty, and clears the unsaved dot", async ({
  page,
}) => {
  await openBuilder(page);
  ...
  await expect(
    page.locator(".nav-row--build").getByTestId("unsaved-dot"),
  ).toBeVisible();
...
test("Revert discards an unsaved choice after a two-step confirm", async ({
  page,
}) => {
  await openBuilder(page);
  await chooseItem(page, "gear.head", HEAD_ITEM);
  ...
  const revert = page
    .locator(".buildbar-actions")
    .getByRole("button", { name: "Revert", exact: true });
  await revert.click();
  await expect(
    page.getByRole("button", { name: "Really revert?" }),
  ).toBeVisible();
