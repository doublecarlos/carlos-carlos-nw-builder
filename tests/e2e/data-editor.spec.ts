...
test("opening and closing the data editor", async ({ page }) => {
  ...
  await expect(
    editor.getByRole("button", { name: /Bonus sets \d+/ }),
  ).toBeVisible();
...
test('deleting an item flags it, feeds the "Edit data" badge, and restore clears it', async ({
  page,
}) => {
  await openBuilder(page);
  await page.getByRole("button", { name: "Edit data" }).click();
  ...
  await expect(
    row.getByTestId("badge").and(page.locator('[data-variant="removed"]')),
  ).toBeVisible();

  await editor.getByRole("button", { name: "✕ Close" }).click();
  await expect(
    page.getByRole("button", { name: "Edit data" }).getByTestId("badge"),
  ).toHaveText("1");

  await page.getByRole("button", { name: "Edit data" }).click();
  await editor.locator(".editor-search").fill(HEAD_ITEM);
  await editor
    .locator(".editor-row", { hasText: HEAD_ITEM })
...
test('a new item\'s bonus card previews its id live from its own name, and a second "Add bonus" never silently no-ops', async ({
  page,
}) => {
  ...
  await expect(cards.nth(1).getByTitle("Assigned when first saved")).toHaveText(
    "cold-resistance-2",
  );
});
