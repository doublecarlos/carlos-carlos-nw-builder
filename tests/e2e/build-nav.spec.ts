// End-to-end coverage for BuildLibrary.vue's sidebar: creating/renaming/duplicating/deleting
...
test("creating a collection adds it to the sidebar and makes it active", async ({
  page,
}) => {
...
test("creating a build in a collection adds a tab and makes it active", async ({
  page,
}) => {
  await openBuilder(page);
  ...
  await collection
    .locator("..")
    .getByRole("button", { name: "+ New build" })
    .click();
...
test("renaming a build via the sidebar updates both the tab and the name field", async ({
  page,
}) => {
  await openBuilder(page);
...
test("duplicating a build via its menu creates a copy and switches to it", async ({
  page,
}) => {
  await openBuilder(page);
...
test("deleting a build needs two clicks, and removes it from the sidebar", async ({
  page,
}) => {
  await openBuilder(page);
  const collection = collectionRow(page, "My builds");
