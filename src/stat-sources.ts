...
export interface StatSource { name: string; value: number }
export interface StatSourceSection { title: string; key: string; sources: StatSource[] }
...
function itemSources(result: ResolvedBuild, key: StatKey): StatSource[] {
  ...
  for (const row of result.rows) {
    ...
    totals.set(row.item.name, (totals.get(row.item.name) ?? 0) + (raw as number));
  }
...
function weaponModSources(result: ResolvedBuild, build: Build | null | undefined, key: StatKey): StatSource[] {
  const out: StatSource[] = [];
  for (const row of result.rows) {
...
function combinedRatingSource(result: ResolvedBuild, key: StatKey): StatSource[] {
...
function ratingContributionSource(result: ResolvedBuild, key: StatKey): StatSource[] {
  const rule = NW_SCHEMA.ratingConversion.find((r) => r.percent === key);
  if (!rule) return [];
  return [{ name: 'Rating contribution', value: result.stages.ratingPct?.[key] ?? 0 }];
}
...
function abilitySource(result: ResolvedBuild, key: StatKey): StatSource[] {
  ...
  return value ? [{ name: NW_SCHEMA.statByKey[rule.ability]?.label ?? rule.ability, value }] : [];
}

/** Stage 6: the forte pool, if the player picked this stat in one of the three forte slots. */
function forteSource(result: ResolvedBuild, build: Build | null | undefined, key: StatKey): StatSource[] {
  const picks = build?.context?.forte as Record<string, string | undefined> | undefined;
...
function sourcesFor(result: ResolvedBuild, build: Build | null | undefined, key: StatKey): StatSource[] {
  return [
...
export function sectionsFor(result: ResolvedBuild, build: Build | null | undefined, key: StatKey): StatSourceSection[] {
  const rule = NW_SCHEMA.ratingConversion.find((r) => r.rating === key);
  if (rule) {
    return [
      { title: 'Rating', key: rule.rating, sources: sourcesFor(result, build, rule.rating) },
      { title: 'Percentage', key: rule.percent, sources: sourcesFor(result, build, rule.percent) },
    ];
  }
