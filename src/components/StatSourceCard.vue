...
<template>
  ...
  <div
    class="statcard fixed z-40 max-h-96 w-64 max-w-[90vw] overflow-y-auto overscroll-contain rounded-md border border-line bg-surface px-2.5 py-2 shadow-lg"
  >
    <div
      class="sticky top-0 -mx-2.5 -mt-2 flex items-baseline justify-between gap-2 bg-surface px-2.5 pb-1 pt-2"
    >
    ...
    <template v-for="(section, i) in sections" :key="section.key">
      ...
      <div v-else class="statcard-empty py-0.5 text-sm text-muted">
