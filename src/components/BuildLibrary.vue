...
const buildById = computed(
  () => new Map(builds.value.map((build) => [build.id, build])),
);

const fsSupported = fsStore.supported;

function buildsIn(collection: Collection): Build[] {
  return collection.buildIds
    .map((id) => buildById.value.get(id))
...
function collectionDirty(collection: Collection) {
  ...
  if (
    ...
    saved.buildIds.some((id, index) => id !== collection.buildIds[index])
  )
    return true;
...
function openMenuFor(type: string, id: string, event: MouseEvent) {
  ...
  const wrap = (event.currentTarget as HTMLElement)
    .closest(".nav-row")
    ?.querySelector(".nav-menu-wrap") as HTMLElement;
...
function onScroll(event: Event) {
  if (!openMenu.value || (event.target as HTMLElement)?.closest?.(".navmenu"))
    return;
  closeMenu();
}
...
function duplicateBuildRow(id: string) {
  library.selectBuildById(id);
  library.duplicateBuild();
}
...
<template>
  <nav
    ref="root"
    class="flex flex-col gap-0.5 overflow-y-auto bg-surface p-2 text-sm"
    data-testid="library"
  >
    <div v-for="collection in collections" :key="collection.id" class="mb-1">
      <div
        class="nav-row nav-row--collection relative flex items-center gap-1 rounded-md px-1 py-1 font-semibold"
        :class="collection.id === activeCollectionId && 'is-active'"
      >
        <button
          type="button"
          class="w-4 flex-none cursor-pointer px-0.5 text-muted"
        ...
        <input
          v-if="isRenaming('collection', collection.id)"
        ...
        <button
          ...
          @contextmenu.prevent="
            openMenuFor('collection', collection.id, $event)
          "
        >
          {{ collection.name }}
        </button>
        ...
        <UnsavedDot
          v-if="collectionDirty(collection)"
        ...
        <div class="nav-menu-wrap relative">
          <button
            ...
            @click="openMenuFor('collection', collection.id, $event)"
          >
            ⋮
          </button>

          <div
            v-if="isMenuOpen('collection', collection.id)"
          ...
          >
            <button
              ...
              @click="startRename('collection', collection.id, collection.name)"
            >
              Rename
            </button>
            <button
              ...
              class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
              @click="
                library.saveCollection(collection.id);
                closeMenu();
              "
            >
              Save
            ...
            <button
              type="button"
              class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
              @click="toggleSaveAs(collection.id)"
            >
              Save As…
            </button>
            <div
              v-if="saveAsFor === collection.id"
              class="my-0.5 ml-2 flex flex-col border-l-2 border-line"
            >
              <button
                type="button"
                class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
                @click="saveAsStorage(collection.id)"
              >
                Browser storage (new copy)
              </button>
              <button
                type="button"
                class="rounded-md px-2 py-1 text-left enabled:cursor-pointer disabled:text-muted enabled:hover:bg-surface-2"
                :disabled="!fsSupported"
                :title="
                  fsSupported
                    ? ''
                    : 'This browser cannot keep a file linked -- use Export instead'
                "
            ...
            <button
              ...
              class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
              @click="
                library.duplicateCollection(collection.id);
                closeMenu();
              "
            >
              Duplicate
            ...
            <button
              ...
              class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
              @click="
                library.exportCollection(collection.id);
                closeMenu();
              "
            >
              Export…
            ...
            <button
              type="button"
              class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-danger-soft hover:text-danger"
              @click="
                runConfirmed(
                  'collection',
                  collection.id,
      ...
      <div
        ...
        class="ml-1.5 border-l border-accent"
      >
        <div
          v-for="build in buildsIn(collection)"
          :key="build.id"
        ...
        >
          <input
            v-if="isRenaming('build', build.id)"
            :ref="setRenameInputRef"
            v-model="renameText"
            class="nav-rename min-w-0 flex-1 rounded-md border border-line bg-surface px-1 py-0.5"
            @keydown.enter="commitRename"
          ...
          <div class="nav-menu-wrap relative">
            <button
              ...
              class="nav-kebab flex-none cursor-pointer rounded-md px-1.5 leading-none text-muted hover:bg-surface-2 hover:text-text"
              title="Build menu"
              @click="openMenuFor('build', build.id, $event)"
            >
              ⋮
            </button>

            <div
              v-if="isMenuOpen('build', build.id)"
              class="navmenu fixed z-30 flex min-w-48 -translate-x-full flex-col rounded-md border border-line bg-surface p-1 shadow-lg"
              :style="{ top: menuPos.top + 'px', left: menuPos.left + 'px' }"
            >
              <button
                type="button"
                class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
                @click="startRename('build', build.id, build.name)"
              >
                Rename
              </button>
              <button
              ...
              <button
                ...
                class="rounded-md px-2 py-1 text-left cursor-pointer hover:bg-surface-2"
                @click="
                  runConfirmed('build', build.id, 'reset-build', () =>
                    resetBuild(build.id),
                  )
                "
              >
              ...
              <button
                ...
                @click="
                  runConfirmed('build', build.id, 'delete-build', () =>
                    deleteBuildRow(build.id),
                  )
                "
              >
        ...
        <div class="nav-row--actions flex items-center gap-2.5 pl-5 text-muted">
          <BaseButton
            variant="link"
            @click="library.createBuildIn(collection.id)"
            >+ New build</BaseButton
          >
          <BaseButton variant="link" @click="triggerImportBuild(collection.id)"
            >Import</BaseButton
          >
        </div>
      </div>
    ...
    <div
      class="nav-row--actions mt-1 flex items-center gap-2.5 border-t border-line pl-1 pt-1.5 text-muted"
    >
      <BaseButton variant="link" @click="library.createCollection()"
        >+ New collection</BaseButton
      >
      <BaseButton variant="link" @click="triggerImportCollection"
        >Import collection</BaseButton
      >
    </div>
