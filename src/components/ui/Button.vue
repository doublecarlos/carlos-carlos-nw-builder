...
const classes = computed(() => {
  if (props.variant === 'link') {
    return ['bg-transparent border-0 cursor-pointer px-1 py-0.5', props.disabled && 'text-muted cursor-default'];
  }
  ...
  else if (props.active || props.variant === 'primary') tone = 'bg-accent-soft border-accent';
