  ...
  <section class="rounded-md border border-line">
    <div class="bg-surface-2 flex items-center">
      <button type="button"
              ...
              :class="isCursor && 'is-cursor outline-2 -outline-offset-1 outline-accent'"
              :data-cursor-key="'header:' + id"
              @click="$emit('toggle')">
        <span class="w-2.5 text-muted">{{ expanded ? '▾' : '▸' }}</span>
        <span class="truncate">{{ label }}</span>
        <span class="ml-auto font-normal text-muted" :class="total > 0 ? 'section-count' : ''">{{ total ? `${filled}/${total}` : '' }}</span>
        <Badge v-if="errors" variant="error">{{ errors }}</Badge>
        <Badge v-if="highlightDiff && diffs" variant="diff">{{ diffs }}</Badge>
        <UnsavedDot v-if="unsaved" title="Unsaved changes in this section" />
      </button>
      <SectionCopyMenu v-if="otherBuilds.length" :section-id="id" :other-builds="otherBuilds"
                       @copy="(fromId) => $emit('copy', fromId)" />
      <IconButton v-if="unsaved" icon="undo-2" title="Revert this section to saved"
                  class="mr-1.5 flex-none" @click="$emit('revert')" />
    </div>

    <div v-if="expanded" class="bg-surface border-t border-line px-2.5 pb-2 pt-1">
      <slot v-for="slot in slots" :key="slot.id" :slot="slot" />
    </div>
  </section>
</template>
