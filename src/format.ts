...
export const int = (value: unknown) => (finite(value) ? GROUPED.format(Math.round(value)) : '—');

/** A decimal fraction as a percentage: 0.09 -> "9.00%". */
export const pct = (value: unknown, digits = 2) => (finite(value) ? `${(value * 100).toFixed(digits)}%` : '—');
...
export const stat = (key: StatKey, value: unknown) => (isPercentKind(kindOf(key)) ? pct(value) : int(value));
...
export const hasBonuses = (item: Item | null | undefined) => Boolean(item && (item.bonuses?.length ?? 0) > 0);

/** `warlock` -> `Warlock`, `artifactCall` -> `Artifact Call`. */
export const titleCase = (value: unknown) => String(value ?? '')
