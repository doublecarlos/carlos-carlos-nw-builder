...
export const int = (value: any) => (finite(value) ? GROUPED.format(Math.round(value)) : '—');

/** A decimal fraction as a percentage: 0.09 -> "9.00%". */
export const pct = (value: any, digits = 2) => (finite(value) ? `${(value * 100).toFixed(digits)}%` : '—');

/** Percent-flavoured stats are the ones stored as fractions. */
export const isPercentKind = (kind: string) => kind === 'percent' || kind === 'mult';

export const kindOf = (key: string) => statByKey()[key]?.kind ?? 'flat';

export const label = (key: string) => statByKey()[key]?.label ?? key;
...
export const abbr = (key: string) => statByKey()[key]?.abbr ?? label(key);

/** Format a stat value the way its kind demands. */
export const stat = (key: string, value: any) => (isPercentKind(kindOf(key)) ? pct(value) : int(value));

/** As `stat`, with an explicit `+` on positive values -- for deltas and item previews. */
export const signedStat = (key: string, value: any) => {
 ...
 */
export const itemPreview = (item: any, limit = 4) => {
...
export const hasBonuses = (item: any) => Boolean(item && (item.bonuses?.length ?? 0) > 0);

/** `warlock` -> `Warlock`, `artifactCall` -> `Artifact Call`. */
export const titleCase = (value: any) => String(value ?? '')
