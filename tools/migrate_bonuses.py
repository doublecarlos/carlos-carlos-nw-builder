...
Reads `data/raw/db-items.json`; writes `data/db-items.json`, `data/db-bonuses.json` and a review
...
def main():
    ...
    jsonemit.write_rows(repo_path('data', 'db-items.json'), items)
    jsonemit.write_rows(repo_path('data', 'db-bonuses.json'), bonus_sets)

    os.makedirs(os.path.dirname(args.report), exist_ok=True)
    write_report(args.report, migration, items, bonus_sets)

    print(f'wrote data/db-items.json   ({len(items)} items)')
    print(f'wrote data/db-bonuses.json ({len(bonus_sets)} sets)')
