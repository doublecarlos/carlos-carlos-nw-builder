"""Generate randomised builds, run them through the legacy oracle, emit `tests/differ-cases.js`.
...
surfaces there -- which keeps the emitted file small enough to load in a browser.
...
def main():
    ...
    target = repo_path('tests', 'differ-cases.js')
    lines = [
        '// GENERATED by tools/gen_differ_cases.py -- do not edit.',
        f'// {len(cases)} randomised builds (seed {args.seed}, fill {args.fill}), each run',
        '// through the LEGACY oracle (tools/legacy_engine.py). tests/differ.html runs the new',
        '// engine over the same builds and reports every divergence.',
        '//',
        '// Items fixed in tools/corrections.py are excluded from the random pool -- they are',
        '// meant to diverge. Every case here must match the oracle exactly.',
        '',
        'window.NW_DIFFER_CASES = [',
        ',\n'.join('  ' + jsemit.value(case, 2, width=200) for case in cases),
        '];',
        '',
    ]
